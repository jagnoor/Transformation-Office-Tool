...
# =============================================================================
# Teaching notes: renderer.py (PDF/PNG chart rendering)
#
# This module draws the roadmap chart using matplotlib.
#
# Big idea:
#   - Dates are mapped to X positions (left-to-right timeline).
#   - Workstreams (and sublanes) map to Y positions (top-to-bottom swimlanes).
#   - Each task becomes a shape (rounded block or diamond milestone).
#
# The renderer is used for:
#   - Preview image in the Streamlit app
#   - High-resolution PNG exports
#   - Vector PDF exports
#
# If you're new to matplotlib:
#   - "fig" is the whole figure (the page)
#   - "ax" is an axis (a drawing area on that page)
#   - patches (Rectangle/FancyBboxPatch/Polygon) are shapes we add to the axis
# =============================================================================
...
def render_roadmap(
    ...
    *,
    ...
    """
    # Rendering process (high level):
    # 1) Pick a font and colors that look good on most machines.
    # 2) Decide which tasks are visible (and clamp partial out-of-range tasks).
    # 3) Create a figure sized like the chosen page (A3/A4 landscape).
    # 4) Compute lane layout (workstream bands + sublane rows).
    # 5) Draw timeline header, grid, lane labels, and task blocks/milestones.
    # 6) Return the figure plus any warnings for the UI to display.
